    ...
    jQuery.ajax({
        ...
        success:function(data) {
            ...
            $.each(data, function (i, item) {
                var status;
                var $d=$('<button class="btn-xs btn-info btn-fill btn-danger btn-pace-right " data-id="'+item.virtualSensorId.toString()+'" toggle="modal" onclick="deleteSensor(event)">Return</button>');
                ...
                $("#user-virtualsensor-table").append($('<tr/>').append($('<td/>').append(item.virtualSensorId)).append($('<td/>').append(item.virtualSensorName)).append($('<td/>').append(item.sensorLocation)).append($('<td/>').append(status)).append($('<td/>').append(item.sensorType)).append($('<td/>').append(item.vendorName)).append($d));
                i++;
            });
        },
    });
...
function addsensorforuser() {
    jQuery.ajax({
        ...
        success:function(data) {
            ...
            $.each(data, function (i,item) {
                var status;
                var $e=$('<button class="btn-xs btn-info btn-fill btn-edit btn-pace-left " data-id="'+item.sensorId.toString()+'" data-name="'+item.sensorName+'" data-type="'+item.sensorType+'" data-status="'+item.sensorStatus+'" data-location="'+item.sensorLocation+'"  toggle="modal" onclick="addvirtualSensor(event)">Add</button>');
...
function addvirtualSensor(event)
{
    ...
    var virsensorId = (getRandom()) + physensorId;
    var virtualSensorName = "Vir_" + virsensorId + event.target.dataset.name
